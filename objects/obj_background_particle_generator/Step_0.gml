function backgroundParticlesGenerate() {
	...
	var star_mov_speed = random_range(0.5, 1);
	var star_angle = random_range(0, 360);
	var star_rotation_speed = random(2);
	...
	var comet_mov_speed = random_range(1.25, 5.55);
	var comet_scale = random_range(0.1, 0.7);
	var comet_alpha_limit = random(0.5)
	var comet_angle = random_range(220, 330);
...
function spaceBiomesChange() {
	...
	if (spaceBiomeChangeTime >= spaceBiomeMaxTime) {
		...
		spaceBiomes = choose("Death Space", "Comets Rain", "Asteroids Sector", "Planets Imperium", "Calm Space", "Final Horizon");
	}
	...
	switch (spaceBiomes) {
		...
		case "Comets Rain":
			biomeCometsRain();
		break;
		case "Asteroids Sector":
			biomeAsteroidsSector();
		break;
		case "Planets Imperium":
			biomePlanetsImperium();
		break;
		case "Final Horizon":
			biomeFinalHorizon();
		break;
